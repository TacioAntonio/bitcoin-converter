...
function convertBTC() {
  ...
  request(url, function (error, response, body) {
    ...
    try {
      if (!response && response.statusCode !== 200) throw new Error('The requisition failed.');

      apiResponse = JSON.parse(body);
    } catch (parseError) {
      console.error(chalk.red('Something went wrong in the API. Try in a few minutes.'));
      return parseError;
    }
